...
async def scrape_sportsbook_fast(
    ...
    run_config = CrawlerRunConfig(
        ...
        page_timeout=30000,  # wait max 30 sec per page to load
        wait_for="css:body", #wait until body tag is present, so make sure there is a page strucutre
        delay_before_return_html=5,  # Wait 3 sec for dynamic content
        ...
        js_code=[
            ...
            "await new Promise(r => setTimeout(r, 1000));", #scroll halfway down the page to trigger lazy loading
...
def save_to_csv(betting_links, filename='betting_links.csv'): #save links to csv file
    ...
    # Create paths for Links and JSON_Files folders
    ...
    # save to CSV with single column in Links folder
    ...
    print(f"Saved {len(betting_links)} links to: {csv_path}") #print msg confirming save
    
    # save full data to JSON for reference in JSON_Files folder
...
async def main():
    ...
    start_url = "https://in.betrivers.com/?page=sportsbook&feed=featured#home"
    ...
    save_to_csv(betting_links, filename='betting_links_betrivers.csv') 
