...
def filter_nba_event_links(csv_file_path, nba_teams):
    """
    Filter links from CSV that include 'nba/' followed by any NBA team name.
    
    Returns:
        list: filtered_links list
    """
...
def scrape_single_game(browser_config, URL, market, POINTS_REBOUNDS_ASSISTS_OU_QUERY):
    """
    ...
    Args:
        ...
        market: Market name
        POINTS_REBOUNDS_ASSISTS_OU_QUERY: AgentQL query
    
    Returns:
        list: Rows of player props data
    """
    # Extract game name from URL
    match = re.search(r'/nba/([^/]+)-\d+', URL)
    if match:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            data = page.query_data(POINTS_REBOUNDS_ASSISTS_OU_QUERY)
            selections = data.get("points_rebounds_assists_ou", [])
...
def main():
    ...
    prophetx_dir = os.path.dirname(script_dir)
    scraper_dir = os.path.dirname(prophetx_dir)
    
    # Path to the betting links CSV file
    csv_file_path = os.path.join(scraper_dir, 'Links', 'betting_links_prophetx.csv')
    
    # Filter NBA event links
    filtered_links = filter_nba_event_links(csv_file_path, nba_teams)
    
    # Add query parameter to filtered links
    filtered_links = [url + "?currency=cash&subType=player_total_points_rebounds_assists" for url in filtered_links]
    ...
    POINTS_REBOUNDS_ASSISTS_OU_QUERY = """
    {
        points_rebounds_assists_ou[] {
            player_name
            line
            odds_over
            odds_under
    ...
    market_match = re.search(r'(\w+)\[\]', POINTS_REBOUNDS_ASSISTS_OU_QUERY)
    ...
    max_workers = min(len(filtered_links), 6)  # Limit to 6 concurrent browsers to avoid resource issues
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all scraping tasks
        futures = {
            executor.submit(scrape_single_game, browser_config, URL, market, POINTS_REBOUNDS_ASSISTS_OU_QUERY): URL 
    ...
    output_file = os.path.join(odds_dir, 'prophetx_nba.csv')
