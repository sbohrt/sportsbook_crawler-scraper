...
def scrape_single_game(browser_config, URL, market, POINTS_REBOUNDS_ASSISTS_QUERY):
    """
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Click Player points O/U button
            page.locator("button").filter(has_text="Player points + rebounds + assists").click()
            
            # Click Show More to expand all player props
            page.locator("ms-player-props-option-group").get_by_text("Show More").click()

            # === SCRAPE MARKET USING AGENTQL ===
            data = page.query_data(POINTS_REBOUNDS_ASSISTS_QUERY)
            selections = data.get("points_rebounds_assists", [])
...
def main():
    ...
    market_match = re.search(r'(\w+)\[\]', POINTS_REBOUNDS_ASSISTS_QUERY)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        futures = {
            executor.submit(scrape_single_game, browser_config, URL, market, POINTS_REBOUNDS_ASSISTS_QUERY): URL 
