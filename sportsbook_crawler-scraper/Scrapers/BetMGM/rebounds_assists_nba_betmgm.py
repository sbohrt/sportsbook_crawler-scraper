...
def scrape_single_game(browser_config, URL, market, REBOUNDS_ASSISTS_QUERY):
    """
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.locator("button").filter(has_text="Player rebounds + assists").nth(1).click()
            page.locator("ms-player-props-option-group").get_by_text("Show More").click()

            # === SCRAPE MARKET USING AGENTQL ===
            data = page.query_data(REBOUNDS_ASSISTS_QUERY)
            selections = data.get("rebounds_assists", [])
...
def main():
    ...
    market_match = re.search(r'(\w+)\[\]', REBOUNDS_ASSISTS_QUERY)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        futures = {
            executor.submit(scrape_single_game, browser_config, URL, market, REBOUNDS_ASSISTS_QUERY): URL 
