...
# Note: Ensure this URL actually leads to the Threes tab, or the script might not find the header.
URL = "https://sportsbook.fanduel.com/basketball/nba/phoenix-suns-@-minnesota-timberwolves-35036256?tab=player-threes"

ACCORDION_QUERY = """
{
    accordion_header(name: "Player Made Threes")
}
"""
...
DATA_QUERY = """
{
    lines_section(name: "Player Made Threes") {
        rows[] {
            player_name
            over_line_label
            over_price
...
def main():
    with sync_playwright() as p:
        ...
        # REPLACEMENT: Wait 5 seconds for auto-load instead of manual input
        ...
        try:
            ...
            if response.accordion_header:
                response.accordion_header.click()
            else:
                raise Exception("AgentQL header miss")
        except Exception:
            # Updated fallback to match the "Threes" context
            try:
                page.get_by_text("Player Made Threes", exact=True).last.click()
            except:
                pass
        ...
        for row in rows:
            ...
            if p_name and line_val:
                csv_rows.append({
                    "player_name": p_name,
                    "threes_line": line_val,
                    "odds_over": o_odds,
                    "odds_under": u_odds
                })

        output_file = "fanduel_threes_ou.csv"
        with open(output_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                f,
                fieldnames=["player_name", "threes_line", "odds_over", "odds_under"]
            )
            writer.writeheader()
